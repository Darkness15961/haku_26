...
final salidaRemotoDataSourceProvider = Provider<SalidaDataSourceSupabase>((
  ref,
) {
...
final salidasRemotasProvider = FutureProvider<List<ModeloSalidaRemota>>((
  ref,
) async {
...
final salidasPorRutaProvider =
    FutureProvider.family<List<ModeloSalidaRemota>, String>((
      ...
      ref.watch(perfilVersionProvider);
      ref.watch(sesionProvider.select((s) => s.usuario?.id ?? ''));
      if (!supabaseListo || rutaId.trim().isEmpty) return const [];
      return ref
          .read(salidaRemotoDataSourceProvider)
          .listarVisibles(rutaId: rutaId);
    });
