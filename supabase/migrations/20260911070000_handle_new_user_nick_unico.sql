-- Google / registro: evitar fallo del trigger si nombre_nick ya existe
-- (mismo local-part de correo en dominios distintos, p. ej. ana@gmail / ana@outlook).
-- Sin esto: auth.users se crea pero public.usuario no → sesión huérfana.
...
begin
  ...
  -- UNIQUE (nombre_nick): si choca, sufijo estable del uuid.
  if exists (
    select 1 from public.usuario u where u.nombre_nick = v_nick
  ) then
    v_suffix := substr(replace(new.id::text, '-', ''), 1, 8);
    v_nick := left(v_nick, greatest(1, 50 - 1 - length(v_suffix))) || '_' || v_suffix;
  end if;

  if exists (
    select 1 from public.usuario u where u.nombre_nick = v_nick
  ) then
    v_nick := 'user_' || substr(replace(new.id::text, '-', ''), 1, 12);
  end if;
...
COMMENT ON FUNCTION public.handle_new_user() IS
  'Tras auth.users INSERT: crea public.usuario. Nick único (sufijo si choca). Google: correo, foto, PE. Editable en Configuración.';
